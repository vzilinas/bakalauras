...
class OfficeNeedsExpenses595da(Bolt, StatefulComponent):
    # Important : Define output field tags for the Bolt
    outputs = ["OfficeNeedsExpenses_595da653-00e2-4af6-b41f-b39c3c03c2fa", "unique_id"]
    ...
    def initialize(self, config, context):
        ...
        self.log("Initializing OfficeNeedsExpenses595da...")
        self.results = {
    ...
    def process(self, tup):
        ...
        if True:
            ...
            if not({'EquipmentExpenses0a185e3', 'WorkingPlaceCost98026c71'} <= set(self.temp_combination[input_dict['unique_id']])):
                return

        input_value = (self.temp_combination[output_dict['unique_id']]['EquipmentExpenses0a185e3']['last_value'] / 1.21) + self.temp_combination[output_dict['unique_id']]['WorkingPlaceCost98026c71']['last_value']
        if output_dict['unique_id'] in self.temp_combination:
        ...
        output_dict['result']['OfficeNeedsExpenses595da'] = result
        self.emit([pickle.dumps(output_dict), output_dict['unique_id']])
        self.redis_db.sadd('departament-expenditure-indicator:OfficeNeedsExpenses595da:state_values', output_dict['primary_key'])
        self.redis_db.set('departament-expenditure-indicator:OfficeNeedsExpenses595da:' + output_dict['primary_key'], msgpack.packb(self.results[output_dict['primary_key']]))
