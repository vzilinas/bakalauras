...
class EquipmentExpenses0a185e3(Bolt, StatefulComponent):
    # Important : Define output field tags for the Bolt
    outputs = ["EquipmentExpenses_0a185e3c-24ff-40e6-83f3-40b1a4f0d56e", "unique_id"]
    ...
    def initialize(self, config, context):
        ...
        self.log("Initializing EquipmentExpenses0a185e3...")
        self.results = {
    ...
    def process(self, tup):
        ...
        if True:
            ...
            if not({'MandatoryEquipmentExpense', 'OptionalEquipmentExpense'} <= set(self.temp_combination[input_dict['unique_id']])):
                return

        input_value = self.temp_combination[output_dict['unique_id']]['MandatoryEquipmentExpense']['last_value'] + self.temp_combination[output_dict['unique_id']]['OptionalEquipmentExpense']['last_value']
        if output_dict['unique_id'] in self.temp_combination:
        ...
        output_dict['result']['EquipmentExpenses0a185e3'] = result
        self.emit([pickle.dumps(output_dict), output_dict['unique_id']])
        self.redis_db.sadd('departament-expenditure-indicator:EquipmentExpenses0a185e3:state_values', output_dict['primary_key'])
        self.redis_db.set('departament-expenditure-indicator:EquipmentExpenses0a185e3:' + output_dict['primary_key'], msgpack.packb(self.results[output_dict['primary_key']]))
