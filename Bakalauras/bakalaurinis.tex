...
Srautinio apdorojimo sistemų kodo generavimas yra vykdomas pagal šablonus, kurie sukurti kiekvienam srautinio apdorojimo sprendimo komponento tipui. Šablono laukai, kuriuos reikia pakeisti, užrašyti specialiais identifikuojančiais elementais. Generavimo metu šie elementai keičiami reikiamu kodu užduočiai spręsti.
...
\begin{itemize}
    ...
    \item Postman programa naudojama išsiųsti rodiklių duomenų modelį į koncepcinio sprendimo generavimo tašką.
    \item Naršyklė naudojama stebėti sugeneruotas ir pateiktas srautinio apdorojimo sistemas per "Heron" sąsają ir gauti duomenis iš apdorotų rodiklių duomenų bazės.
\end{itemize}
...
Taip pat tarkime, kad po laiko buvo nuspręsta taip pat apdoroti bendro pojūčio rodiklį: 
...
\begin{itemize}
    ...
    \]
\end{itemize}
Pakeistas rodiklių duomenų modelis atrodo taip:
\begin{itemize}
    \item Pirminis raktas: \(\textit{"Valanda", "Kabinetas"}\)
    \item Pirminių rodiklių duomenų apribojimai: \(\textit{"Valanda" > 7},\textit{"Valanda" < 21}\) 
    \item Rodikliai:
...
\begin{enumerate}
    ...
    \item Atsidarę "Heron" sąsają naršyklėje matome srautinio apdorojimo sistemos diagramą (Priedas \ref{add:generated-system1}).
    ...
    \item Pakeistas rodiklių duomenų modelis užrašomas JSON formatu (Priedas \ref{add:changed-json}) ir naudojant Postman programą paduodamas į sprendimą.
...
\begin{enumerate}
    ...
    \item Atlikta srautinio apdorojimo sprendimų analizė ir pasirinktas tinkamas srautinių rodiklių duomenų apdorojimo sistemų generavimui. 
    \item Sudaryta srautinio apdorojimo sistemos architektūra tinkama spręsti rodiklių apdorojimo uždavinį. 
    \item Sukurtas koncepcinis sprendimas pagal sudaryta architektūrą.
...
\section{Generuojami pirminių rodiklių duomenys}\label{add:mockaroo}
\begin{figure}[H]
    \centering
...
\section{Pradinis rodiklių duomenų modelis JSON formatu}\label{add:initial-json}
\lstset{
    string=[s]{"}{"},
...
\section{Pradinė sugeneruota sistema "Heron" sąsajoje}\label{add:generated-system1}
\begin{figure}[H]
    \centering
...
\section{Dalis pradinės sugeneruotos sistemos rezultatų}\label{add:generated-data1}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{img/topology-data-1.png}
    \label{img:generated-data}
\end{figure}
\section{Pakeistas rodiklių duomenų modelis JSON formatu}\label{add:changed-json}
\begin{lstlisting}
{
    "IndicatorId": "32c24420-afbd-44fb-b045-ef72c48cb04e",
    "Name": "office-sensors-indicator",
...
\section{Pakeista sugeneruota sistema "Heron" sąsajoje}\label{add:generated-system2}
\begin{figure}[H]
    \centering
...
\section{Dalis pakeistos sugeneruotos sistemos rezultatų}\label{add:generated-data2}
\begin{figure}[H]
    \centering
